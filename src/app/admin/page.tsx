...
import { getCockpitDataSafe, BlockStatus } from "@/lib/superadmin/cockpit-data-adapter";
...
export default async function AdminDashboard() {
    console.log("[COCKPIT_V3.2] Rendering Operational Ready UI");
    
    // 0. Data via v3.2 Robust Adapter
    const { blocks, systemStatus, loadTimeMs } = await getCockpitDataSafe();
    const isSafeMode = systemStatus === 'safe_mode';

    // 1. Audit Action (Deterministic)
    if (systemStatus === 'operational') {
        try {
            ...
            if (user) {
                await CockpitService.auditAdminAction(user.id, 'SUPERADMIN_COCKPIT_VIEWED', `v3.2 Operational Ready (load: ${loadTimeMs}ms)`);
            }
        } catch {
            console.warn("[COCKPIT_V3.2] Non-blocking audit failure");
        }
    }
    ...
    return (
        <div className="space-y-10 animate-in fade-in duration-700 pb-12" data-testid="superadmin-cockpit-root">
            ...
            {/* 0. High-Fidelity Diagnostic Banner */}
            {isSafeMode && (
                <div 
                ...
                >
                    ...
                    <div>
                        <h3 className="text-amber-900 dark:text-amber-100 font-black italic tracking-tight uppercase text-xs mb-1">Entorno en Modo Seguro (Degradado)</h3>
                        <p className="text-amber-700/80 dark:text-amber-300/80 text-[11px] leading-relaxed font-medium max-w-2xl">
                            Falta la configuración de <span className="font-bold">Nivel de Servicio</span> (Admin Key). El Cockpit ha desactivado la consolidación multi-tenant para garantizar la estabilidad del sistema. Complete la configuración en el panel de control de Vercel para activar el modo completo.
                        </p>
                    </div>
                </div>
            )}
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 border-b border-border/50 pb-8">
                <div>
                    <div className="flex items-center gap-4 mb-2">
                        ...
                        <h1 className="text-4xl font-black text-foreground tracking-tighter bg-gradient-to-r from-blue-700 to-indigo-500 bg-clip-text text-transparent italic uppercase">Global Cockpit v3.2</h1>
                    </div>
                    <p className="text-muted-foreground font-medium underline decoration-blue-500/20 underline-offset-8 tracking-tight italic">Operaciones maestras y gobernanza estratégica en tiempo real.</p>
                </div>
            ...
            <section className="min-h-[80px]" data-testid="cockpit-alerts-card">
                <BlockContainer status={blocks.alerts.status} label="Alertas de Producción">
            ...
            <div className="bg-zinc-950 rounded-[3rem] p-10 text-white overflow-hidden relative shadow-2xl border border-white/5 shadow-blue-500/10 group">
                <div className="relative z-10 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 text-center md:text-left">
                    <KPICell label="Estado del Engine" status={blocks.kpis.status}>
                        <div className="flex items-center justify-center md:justify-start gap-3">
                            <Activity className={cn("w-5 h-5", blocks.kpis.status === 'ok' ? 'text-emerald-400' : 'text-zinc-600')} />
                            <span className="text-2xl font-black italic uppercase tracking-tighter">
                                {blocks.kpis.status === 'ok' ? 'Online' : 'Standby'}
                            </span>
                        </div>
                    </KPICell>
                    <KPICell label="Facturas con Riesgo" value={blocks.kpis.data.issuesCount} status={blocks.kpis.status} color="text-rose-400" />
                    <KPICell label="Incidentes Activos" value={blocks.alerts.data.length} status={blocks.alerts.status} color="text-blue-400" />
                    <KPICell label="Organizaciones" value={blocks.kpis.data.totalOrgs} status={blocks.kpis.status} />
                </div>
            ...
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-10">
                <div className="xl:col-span-2" data-testid="cockpit-metrics-card">
                    <BlockContainer status={blocks.metrics.status} label="Rendimiento del Ecosistema" height="min-h-[400px]">
                        <SuperadminMonthlyMetrics data={blocks.metrics.data} />
                    </BlockContainer>
                </div>
                <div className="space-y-6">
                    {stats.map((stat, i) => (
                        <div key={i} data-testid={stat.id} className={cn(
                            ...
                            <div className="flex items-center gap-5 relative z-10">
                                ...
                                <div>
                                    ...
                                    {stat.status !== 'ok' && (
                                        <p className="text-[8px] text-amber-600 font-bold mt-1.5 uppercase tracking-widest italic">Configuración Requerida</p>
                                    )}
                                </div>
                            </div>
                        </div>
            ...
            <div className="space-y-6 p-1" data-testid="cockpit-orgs-table">
                <BlockContainer status={blocks.orgs.status} label="Directorio Maestro de Empresas">
                    <SaaSHealthTable orgs={blocks.orgs.data} />
                </BlockContainer>
            </div>

            {/* 5. Master Navigation Links */}
...
function BlockContainer({ children, status, label, height = "min-h-[100px]" }: { children: React.ReactNode, status: BlockStatus, label: string, height?: string }) {
        ...
        return (
            <div className={cn("w-full flex items-center justify-center bg-zinc-50/50 dark:bg-zinc-900/20 border-2 border-dashed border-border rounded-[3rem] p-12 transition-all hover:border-zinc-300 dark:hover:border-zinc-800 group/block", height)}>
                <div className="text-center space-y-4">
                    <div className="w-16 h-16 bg-white dark:bg-zinc-800 rounded-3xl flex items-center justify-center mx-auto shadow-sm border border-border transition-transform group-hover/block:scale-110">
                    ...
                    <div>
                        ...
                        <p className="text-[10px] text-muted-foreground font-medium italic">
                            {status === 'degraded_config' ? 'Disponible al completar configuración del servidor' : 'Servicio temporalmente no disponible'}
                        </p>
                    </div>
                </div>
            </div>
...
function KPICell({ label, value, status, color = "text-white", children }: { label: string, value?: number, status: BlockStatus, color?: string, children?: React.ReactNode }) {
    return (
